...
def accuracy(model, images, positives, negatives, batch_size, width, height, threshold=0.85):
    positive_distances = []
    negative_distances = []
    ...
    x_batch_one = np.zeros((batch_size, 1, height, width))
    x_batch_two = np.zeros((batch_size, 1, height, width))

    # . . matching images
    # . . number of batches in the data
    num_batches = int(np.ceil(len(positives) / batch_size))

    for i in range(num_batches):
        print(f"positive batch: {i+1}/{num_batches}")
        positive_batch_indices = positives[i*batch_size: (i+1)*batch_size]

        # . . fill the x_batch and y_batch
        j = 0
        for idx1, idx2 in positive_batch_indices:
            x_batch_one[j,0] = images[idx1]
            x_batch_two[j,0] = images[idx2]
            j += 1
        ...
        # . . compute the distances between image pairs
    ...
    for i in range(num_batches):
        print(f"negative batch: {i+1}/{num_batches}")
        negative_batch_indices = negatives[i*batch_size: (i+1)*batch_size]

        # . . fill the x_batch and y_batch
        j = 0
        for idx1, idx2 in negative_batch_indices:
            x_batch_one[j,0] = images[idx1]
            x_batch_two[j,0] = images[idx2]
            j += 1
    ...
    # . . compute the sensitivity (tpr) and the specificity (tnr)        
    tpr = tp / (tp + fn)
    tnr = tn / (tn + fp)
